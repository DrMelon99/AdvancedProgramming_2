...
```
✅ 플레이어의 스탯 표시
    - 지능(+ 지능 버프), 힘(+ 힘 버프), 스태미나%
```
3. 야간운동
```
🔨 0번 플레이어가 탈락하면 게임 자동 진행
✅ 아이템과 플레이어를 무작위로 배치
    - 아이템과 플레이어는 각각 다른 위치에 배치
    - 아이템의 종류는 배치 될때 결정이 되어있어야 함
    - 아이템의 표시를 'I'에서 '$'로 바꿈
🔨 나머지 플레이어는 <아이템 / 아이템을 소지한 플레이어> 를 향해 이동
    -🔨 주기는 랜덤으로, 적당히 맞출 것
🔨 아이템 획득 (인접 칸)
    ...
    - 아이템을 교환하는 경우, 내려놓은 아이템은 플레이어 반경 3칸 이내 범위의 위치로 흩어짐 (아이템을 던지는 느낌으로)
    - 아이템을 교환하지 않는 경우, 아이템을 줍지 않고 그대로 둠
    - 플레이어는 아이템을 획득하는 시점에서 아이템의 스태미나 버프 만큼 스태미나 회복
    -🔨 상호작용 결과를 다이얼로그에 표시
✅ 플레이어끼리 만났을 경우 (인접 칸)
    - 인접한 플레이어 모두 아이템을 가지고 있지 않다면 서로 무시함
    - 아이템이 없는 플레이어는 아이템을 소지한 플레이어와 인접할 시 <강탈 / 회유 / 무시>중 하나를 택함
    - 0번 플레이어의 경우
    ...
    - 공격 옵션
        ...
        <회유 : Intercept>
            마주친 플레이어 중 한명이라도 아이템을 가지고 있을 시 발생 가능
        <무시 : Ignor>
            마주친 플레이어 중 아무도 아이템을 가지고 있지 않을 시 발생 및 플레이어 결정에 따라 발생
✅ 아이템 및 플레이어와 상호작용을 한 플레이어는 1초 동안 움직임 외의 상호작용을 쉼 (연속 상호작용을 방지하기 위함 -> 쿨타임) 
🔨 아이템 및 플레이어의 상호작용 결과를 다이얼로그에 표시
```
4. 줄다리기
```
⚙️ 맵
...
⚙️ 상태창에는 현재 라운드와 선택하고 있는 플레이어를 보여줌
⚙️ 제비를 고르는 순서는 매 라운드마다 랜덤으로 설정
⚙️ 좌우 화살표로 제비를 고르고, 스페이스 바로 제비를 뽑음
    - '?': 제비
    - '@': 뽑으려 하는 제비
⚙️ n번째 플레이어가 제비를 고르는 순간에 <통과 / 탈락> 결과를 다이얼로그를 통해 출력
    - 통과일 경우: 선택된 제비를 화면에서 제거하고 나머지 제비로 라운드 계속 진행
    - 탈락일 경우: 선택된 제비를 화면에서 제거하고 나머지 제비를 섞은 후 다음 라운드 진행

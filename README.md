...
✅ 다이얼로그 동작 매커니즘 및 구조 변경
    ...
    - 선택 옵션을 순환구조로 자연스럽게 표현
🔨 0 번 플레이어 이름 확인 및 편집
✅ 게임 4종류를 각자 다른 소스파일로 구분
✅ 게임 내 모든 맵의 외각을 '#'에서 '*'로 모두 바꿈
🔨 기존 "무궁화 꽃이 피었습니다" 구동 지원
📋 프로젝트 리팩토링
✅ 디버그 상태 창 추가
✅ 그 어떠한 게임에서도 'q'를 누르면 강제종료 후 다음 장면으로 넘어감
```
1. 플레이어 및 아이템 관련
```
...
✅ NPC는 <70% 왼쪽, 10% 제자리, 10% 위, 10% 아래> 확률로 움직임
    - 영희가 뒤를 돌아보고 있을 때에는 <10% 왼쪽, 70% 제자리, 10% 위, 10% 아래> 확률로 움직임
🔨 <탈락, 통과, 이동가능> 조건
    - <이동가능>
